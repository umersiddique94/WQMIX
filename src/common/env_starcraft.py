...
class Env(MultiAgentEnv):
    ...
    def __init__(
        ...
        self.input_size = env_info["obs_shape"]
        self.n_actions = env_info["n_actions"]
        self.max_steps = env_info['episode_limit']
        self.T = 8*self.max_steps
        self.n_agent = env_info["n_agents"]
        self.fp = np.ones((self.n_agent, self.n_actions)) / self.n_actions
        self.GAMMA = 0.99
        self.n_episode = int(12000000//env_info['episode_limit'])
...
#        return self.get_obs(), self.get_state()
    ...
    def step(self, actions):
        #my edit (mz) ignore available actions:
        actions = deepcopy(actions)
        for i in range(self.n_agent):
            avail = self.get_avail_agent_actions(i)
            if avail[actions[i]] == 0:
                actions[i] = 1 if self.get_unit_by_id(i).health > 0 else 0 #stop if alive, no-op if dead
        ...
        terminated = False
        reward = self.reward_battle(sc_actions)
        ...
        if game_end_code is not None:
            ...
            if game_end_code == 1 and not self.win_counted:
                ...
                if not self.reward_sparse:
                    reward += self.reward_win / float(self.n_agent)
                else:
                    reward = 1 / float(self.n_agent)
            elif game_end_code == -1 and not self.defeat_counted:
                self.defeat_counted = True
                if not self.reward_sparse:
                    reward += self.reward_defeat / float(self.n_agent)
                else:
                    reward = -1 / float(self.n_agent)
    ...
    def reward_battle(self, sc_actions):
        ...
        delta_ally = np.zeros(self.n_agent)
        delta_deaths = np.zeros(self.n_agent)
        delta_enemy = np.zeros(self.n_agent)
            ...
            if not self.death_tracker_ally[al_id]:
                ...
                if al_unit.health == 0:
                    ...
                    if not self.reward_only_positive:
                        delta_deaths[al_id] -= (self.reward_death_value * neg_scale) / float(self.n_agent)
                    delta_ally[al_id] += (prev_health * neg_scale) / float(self.n_agent)
                else:
                    # still alive
        ...
        for e_id, e_unit in self.enemies.items():
            if not self.death_tracker_enemy[e_id]:
                ...
                #my edit (mz) indiv rewards
                #search the source
                sources=[]
                for j, ac in enumerate(sc_actions):
                    if ac.action_raw.unit_command.ability_id == actions["attack"] and ac.action_raw.unit_command.target_unit_tag == e_unit.tag:
                        sources.append(j)

                if len(sources) == 0:
                    if e_unit.health == 0:
                        self.death_tracker_enemy[e_id] = 1
                        delta_deaths += self.reward_death_value / float(self.n_agent)
                        delta_enemy += prev_health / float(self.n_agent)
                    else:
                        delta_enemy += (prev_health - e_unit.health - e_unit.shield) / float(self.n_agent)
                else:
                    if e_unit.health == 0:
                        self.death_tracker_enemy[e_id] = 1
                        delta_deaths[sources] += self.reward_death_value / float(len(sources))
                        delta_enemy[sources] += prev_health / float(len(sources))
                    else:
                        delta_enemy[sources] += (prev_health - e_unit.health - e_unit.shield) / float(len(sources))
        ...
        for i in range(self.n_agent):
            ...
            if unit.health > 0:
                ...
                for j in range(self.n_agent):
                    if i != j:
                        unit2 = self.get_unit_by_id(j)
                        if unit2.health > 0:
                            dist = self.distance(unit.pos.x, unit.pos.y, unit2.pos.x, unit2.pos.y)
                            if dist < sight_range:
                                candidate.append((dist, j))
                candidate.sort()
                for (k, (_,n))  in enumerate(candidate):
                    if k < 2:#limit to 2 neighbors for now (mz)
                        lneigh.append(n)
            neighbors.append(lneigh)

        #ensure stable neighbors
        for i in range(len(neighbors)):
            for neigh in neighbors[i]:
                if i not in neighbors[neigh]:
                    neighbors[neigh].append(i)
